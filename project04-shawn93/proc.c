...
fork(void)
{
  ...
  *np->tf = *proc->tf;
  ...
  //shared memory
  if(proc->sharedm) {
    if (allocpg(np->pgdir) == 0) {
      cprintf("child alloc failed.");
    } else {
      proc->sharedm->ref_count++;
      np->sharedm = proc->sharedm; 
    }
  }
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        if (p->sharedm) {
          p->sharedm->ref_count--;
          if (p->sharedm->ref_count == 0) {
            freevm(p->pgdir);
            kfree((void *) p->sharedm);
          }
          p->sharedm = (void *) 0;
          goto exit;
        } else {
